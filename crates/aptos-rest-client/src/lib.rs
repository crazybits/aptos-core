...
impl Client {
    ...
    pub async fn get_table_rows_bcs_at_version<K: Serialize + DeserializeOwned, V: DeserializeOwned>(
    &self,
    table_handle: AccountAddress,
    version: u64,
) -> AptosResult<Response<Vec<(K, V)>>> {
    let url = self.build_path(&format!("tables/{}/rows?ledger_version={}", table_handle, version))?;
